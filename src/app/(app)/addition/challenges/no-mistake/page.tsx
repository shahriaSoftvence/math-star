...
const ChallengeStartScreen = ({ title, description, onStart, onCancel }: { title: string, description: string, onStart: () => void, onCancel: () => void }) => (
  <div className="w-full min-h-screen relative bg-gradient-to-b from-blue-50 to-purple-50 flex flex-col justify-center items-center p-4">
    <div className="w-full max-w-[450px] min-[516px]:max-w-[600px] max-h-[332px] p-7 bg-white rounded-3xl shadow-lg flex flex-col items-center text-center gap-6 max-[514px]:w-[100%] max-[514px]:w-[100%]">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2 text-base font-normal font-Poppins leading-relaxed max-w-[608px] mx-auto max-[514px]:w-[100%] max-[514px]:w-[100%]">{description}</p>
      </div>
...
const GameResultScreen = ({ score, questionsAnswered, onRetry, onHome }: { score: number, questionsAnswered: number, onRetry: () => void, onHome: () => void }) => (
    <div className="w-full min-h-screen relative bg-gradient-to-b from-blue-50 to-purple-50 flex flex-col justify-center items-center p-4">
        <div className="w-full max-w-md p-8 bg-white rounded-3xl shadow-lg flex flex-col items-center text-center gap-4 min-w-[672px] max-[704]:min-w-[400px] mx-auto">
            ...
            <div>
                <p className="text-xl text-gray-600 leading-7">Final Score: <span className="font-bold text-blue-600">{score}</span></p>
            ...
            <div className="w-full mt-4 flex justify-center items-center gap-4">
                <button onClick={onRetry} className="flex-1 py-2.5 bg-blue-500 text-slate-50 rounded-md font-medium text-sm leading-tight hover:bg-blue-600 transition-colors">Try Again</button>
...
const Numpad = ({ onNumberClick, onBackspace, onSubmit }: { onNumberClick: (num: string) => void; onBackspace: () => void; onSubmit: () => void; }) => {
    ...
    return (
      <div className="grid grid-cols-3 gap-4 w-96">
        {buttons.map((btn) => (
          <button key={btn} onClick={() => handleNumberClick(btn)} className="h-24 text-3xl font-bold text-blue-800 bg-blue-100 rounded-2xl transition-colors hover:bg-blue-200">
            {btn}
          </button>
        ))}
        ...
        <button onClick={() => handleNumberClick('0')} className="h-24 text-3xl font-bold text-blue-800 bg-blue-100 rounded-2xl transition-colors hover:bg-blue-200">0</button>
        ...
        <div className="flex items-start gap-4 md:gap-6 mb-12 md:mb-16">
            ...
            <div className="flex flex-col gap-2 md:gap-3">
                ...
                <p className="text-black text-xl md:text-2xl font-medium font-Nunito leading-snug">
                    The challenge ends on your first mistake, You have 5 seconds for each problem.
                </p>
            </div>
        </div>
