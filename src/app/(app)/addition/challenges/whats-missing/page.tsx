...
export default function WhatsMissingPage() {
  ...
  const [addWhatsMissing] = useAddAdditionWhatsMissingMutation();
  ...
  const handleContinue = async () => {
    try {
      ...
      router.push("/addition"); // navigate after sending
    } catch (error) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    else if (question.missingIndex === 1) correctAnswer = question.num2;
    else correctAnswer = question.answer;
