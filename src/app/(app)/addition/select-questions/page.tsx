...
const QuestionCountCard = ({ count, range }: { count: number, range: string | null }) => (
  <Link href={`/addition/practice?count=${count}&range=${range || '10'}`}>
    <div className="w-38 self-stretch p-8 bg-gradient-to-br from-blue-100 to-blue-200 rounded-xl shadow-md inline-flex flex-col justify-center items-center gap-2 cursor-pointer hover:from-blue-200 hover:to-blue-300 transition-all transform hover:scale-105">
...
function SelectQuestionsContent() {
  ...
  const range: string | null = typeof rangeRaw === 'undefined' ? null : rangeRaw;

  return (
    <div className="max-w-4xl mx-auto">
        <div className="w-full flex justify-start">
          <Link href="/addition" className="text-gray-800 text-[20px] font-bold flex justify-center items-center mb-4">
            <ArrowLeft /> Go Back
          </Link>
        </div>
