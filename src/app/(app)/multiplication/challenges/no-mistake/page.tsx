...
const GameResultScreen = ({ score, questionsAnswered, onRetry, onHome }: { score: number, questionsAnswered: number, onRetry: () => void, onHome: () => void }) => (
    <div className="w-full min-h-screen relative bg-gradient-to-b from-green-50 to-purple-50 flex flex-col justify-center items-center p-4">
        <div className="w-full max-w-md p-8 bg-white rounded-3xl shadow-lg flex flex-col items-center text-center gap-4 min-w-[672px] mx-auto">
            ...
            <div>
                <p className="text-xl text-gray-600 leading-7">Final Score: <span className="font-bold text-green-600">{score}</span></p>
            ...
            <div className="w-full mt-4 flex justify-center items-center gap-4">
                <button onClick={onRetry} className="flex-1 py-2.5 bg-green-500 text-slate-50 rounded-md font-medium text-sm leading-tight hover:bg-green-600 transition-colors">Try Again</button>
...
export default function NoMistakePage() {
   ...
   return (
    <div className="w-full min-h-screen bg-gradient-to-b from-green-50 to-purple-50 p-4 md:p-6">
        <div className="flex items-start gap-4 md:gap-6 mb-12 md:mb-16">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-green-100 rounded-full flex justify-center items-center">
                <Target className="w-10 h-10 text-green-600" />
            </div>
