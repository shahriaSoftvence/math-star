    ...
    <div className="w-full min-h-screen relative bg-gradient-to-b from-pink-50 to-purple-50 flex flex-col justify-center items-center p-4">
        <div className="w-full max-w-md p-8 bg-white rounded-3xl shadow-lg flex flex-col items-center text-center gap-4 min-w-[672px] max-[704]:min-w-[400px] mx-auto">
            ...
            <div className="w-full mt-4 flex justify-center items-center gap-4">
                <button onClick={onRetry} className="flex-1 py-2.5 bg-pink-500 text-slate-50 rounded-md font-medium text-sm leading-tight hover:bg-pink-600 transition-colors">Try Again</button>
...
const Numpad = ({ onNumberClick, onBackspace, onSubmit }: { onNumberClick: (num: string) => void; onBackspace: () => void; onSubmit: () => void; }) => {
    ...
    return (
      <div className="grid grid-cols-3 gap-4 w-96">
        {buttons.map((btn) => (
          <button key={btn} onClick={() => handleNumberClick(btn)} className="h-24 text-3xl font-bold text-pink-800 bg-pink-100 rounded-2xl transition-colors hover:bg-pink-200">
            {btn}
          </button>
        ))}
        ...
        <button onClick={() => handleNumberClick('0')} className="h-24 text-3xl font-bold text-pink-800 bg-pink-100 rounded-2xl transition-colors hover:bg-pink-200">0</button>
                ...
                <div className="w-full md:w-[400px] lg:w-[450px] flex flex-col gap-4 md:gap-5">
                    <div className="self-stretch flex justify-between items-center">
                        <div className="h-16 flex flex-col">
                            ...
                            <span className="text-pink-600 text-2xl md:text-3xl font-bold font-Nunito leading-relaxed">{score + 1}</span>
                        </div>
                        <div className="h-16 flex flex-col">
