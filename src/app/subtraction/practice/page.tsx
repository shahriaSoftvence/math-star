  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <div className="grid grid-cols-3 gap-4">
        {buttons.map((btn) => (
          <button key={btn} onClick={() => onNumberClick(btn)} className="h-16 text-xl font-bold text-pink-800 bg-pink-100 rounded-lg transition-colors hover:bg-pink-200">
            {btn}
          </button>
        ))}
        ...
        <button onClick={() => onNumberClick('0')} className="h-16 text-xl font-bold text-pink-800 bg-pink-100 rounded-lg transition-colors hover:bg-pink-200">
          0
        </button>
...
export default function PracticePage() {
  ...
  useEffect(() => {
    const newQuestions: Question[] = Array.from({ length: questionCount }, () => {
      ...
      const num2 = Math.floor(Math.random() * (num1 + 1)); // Ensure num2 is not greater than num1
